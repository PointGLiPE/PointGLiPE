...
def axis_to_matrix(axis, angle):
    axis = axis.view(1, 3)
    axis = F.normalize(axis, dim=-1)
    A = torch.zeros(axis.shape[0], 3, 3, device=axis.device)
    A[:, 0, 1] = -axis[:, 2]
    A[:, 0, 2] = axis[:, 1]
    A[:, 1, 0] = axis[:, 2]
    A[:, 1, 2] = -axis[:, 0]
    ...
    return M.transpose(-2, -1).contiguous().view(3, 3)
...
class ScanNetV2(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mix = True if self.train and random.random() < get_mix_prob() else False
        if mix:
            x2, c2, n2, l2 = self.datas[random.randrange(0, len(self.datas))]
        ...
        if self.train:
            ...
            if mix:
                ...
                xyz[:, 2] -= xyz[:, 2].min()
                x2[:, 2] -= x2[:, 2].min()
                pick1 = random.choice(xyz)
                pick2 = random.choice(x2)
                x2[:, :2] += pick1[:2] - pick2[:2]

                xyz = torch.cat([xyz, x2], dim=0)
                norm = torch.cat([norm, n2], dim=0)
                col = torch.cat([col, c2], dim=0)
                lbl = torch.cat([lbl, l2], dim=0)

            xmin = xyz.min(dim=0)[0]
            xyz -= xmin
            if mix:
                pick1 -= xmin
            else:
    ...
    def get_test_item(self, idx):
        ...
        xyz -= xyz.min(dim=0)[0]
        feature = torch.cat([col, xyz[:, 2:] * (1 / 2.6), norm], dim=1)
    ...
    def knn(self, xyz: torch.Tensor, grid_size: list, k: list, indices: list, full_xyz: torch.Tensor = None):
        ...
        last = len(k) == 1
        parent_xyz = xyz if not first else None
        gs = grid_size.pop()
        if first:
            full_xyz = xyz
        else:
            ...
            xyz = xyz[sub_indices]
            indices.append(sub_indices)  # 下采样点

        kdt = KDTree(xyz)
        indices.append(kdt.knn(xyz, k.pop(), False)[0])  # 对自己索引

        if not first: # 因为第一次没有下采样
            indices.append(kdt.knn(parent_xyz, 1, False)[0].squeeze(-1))
...
def fix_indices(indices, cnt1: list, cnt2: list):
    """
    fix so ok for indexing as a whole  修正索引，本来是一块一块的，现在需要汇集到一起
    indices：一个存放各层索引的列表（比如 [knn1, sub1, knn2, sub2, knn3, …]）
    cnt1：每层点的数量（或累积数量），用于计算偏移量
    cnt2：修正后的累计偏移量列表
    """
    first = len(cnt2) == 0  # cnt2每次都是重置的，开始就是0
    last = len(cnt1) == 1 or (len(cnt1) == 2 and len(cnt2) != 0)  # cnt1是5个累加的结果，用一次拿走一个
    # len(cnt1) == 2 and len(cnt2) != 0 虽然cnt1还有两次，但是cnt2的长度不为0，也就是已经有数值了
    if first:
        c1 = cnt1[-1]  # 第一次肯定是没有偏置的，0
    else:
    ...
    knn.add_(c1)
    if not first:
        indices.pop().add_(c1)  # 这一个是新加的，用在upsample中的偏置
...
def scan_collate_fn(batch):
    ...
    depth = 5  # (len(indices[0]) + 2) // 3  # 6   这个数量就是一共几层的点云
    cnt1 = [0] * depth
    pts = []

    for ids in indices:
        ind = [0, 2, 5, 8, 11]
        pts.extend(ids[i].shape[0] for i in ind)
        # pts.extend(x.shape[0] for x in ids[:2*depth:2])
        # =====修正索引，就是加上之前点的偏置===
        cnt2 = []  # 每次重置
        fix_indices(ids[::-1], cnt1[::-1], cnt2)  # 每次cnt2都是累加的，
        cnt1 = cnt2  # 保存之前的偏置和结果

    xyz = torch.cat(xyz, dim=0)
    col = torch.cat(col, dim=0)
    lbl = torch.cat(lbl, dim=0)
    indices = [torch.cat(ids, dim=0) for ids in zip(*indices)]
    pts = torch.tensor(pts, dtype=torch.int64).view(-1, 5).transpose(0, 1).contiguous()
