...
class MultiScaleAttentionPE(nn.Module):
    ...
    def forward(self, xyz0, indices_nn, indices, pts_list):
        ...
        # 映射 pts_list -> pts4..0（按你原来的映射）
        pts4 = pts_list[0] if pts_list is not None else None  # 这个是用于获取全局的特征。

        # ---------------- Step top level: level4 (最小集合) ----------------
        if pts4 is not None:
            # group_ids 在 GPU 上创建一次（长度 N4），之后释放
            group_sizes = torch.as_tensor(pts4, dtype=torch.long, device=device)
            # repeat_interleave 用 long 索引
            group_ids = torch.repeat_interleave(torch.arange(len(group_sizes), device=device, dtype=torch.long),
                                                group_sizes)
            # 计算 group-wise max （就地 scatter_reduce_）
            group_max = torch.full((len(group_sizes), C), -float('inf'), device=f4.device, dtype=f4.dtype)
