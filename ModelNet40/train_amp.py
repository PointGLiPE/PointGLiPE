...
resume = False
if resume:
    start_epoch = util.load_state(f"output/model/{cur_id}/last.pt", model=model, optimizer=optimizer, scalar=scalar)[
        "start_epoch"]
else:
    start_epoch = 0
...
for i in range(start_epoch, epoch):
    ...
    util.save_state(f"output/model/{cur_id}/last.pt", model=model, optimizer=optimizer, scalar=scalar,
                    start_epoch=i + 1)
